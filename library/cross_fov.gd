...
static func get_fov_map(
	...
	fov_data.min_y = _get_end_point(
			source, Vector2i.UP, is_obstacle, is_obstacle_args
	).y
	fov_data.min_x = _get_end_point(
			source, Vector2i.LEFT, is_obstacle, is_obstacle_args
	).x

	fov_data.max_y = _get_end_point(
			source, Vector2i.DOWN, is_obstacle, is_obstacle_args
	).y
	fov_data.max_x = _get_end_point(
			source, Vector2i.RIGHT, is_obstacle, is_obstacle_args
	).x
...
static func _get_end_point(
	...
	coords = CastRay.get_coords(
			source, source + direction, is_obstacle,
			is_obstacle_args
	)
...
class FovData:
	...
	func _init(
			half_width_: int, up_: int, right_: int, down_: int,
			left_: int
	) -> void:
