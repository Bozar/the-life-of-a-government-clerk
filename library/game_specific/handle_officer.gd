...
static func set_active(
	...
	for i in officer_states:
		...
		# `officer_records` is empty when the very first Document is
		# delivered. It should NOT be empty once again outside `if`
		# statements. Push another `id` into `officer_records` because
		# this Officer has already been selected twice:
		#> (1) Being hit by PC;
		#> (2) Being selected by `set_active()`.
		if officer_records.is_empty():
			officer_records.push_back(id)
			_save_record(i, officer_records)
			has_active_officer = true
		# Add an element into `officer_records` right after it is
		# cleared. DO NOT let an empty `officer_records` go into the
		# next loop.
