...
static func _set_wizard_mode(setting: Dictionary) -> void:
	var setting_value: SettingValue = _parse_setting(
			setting, WIZARD_MODE, VALUE_TYPE.BOOL
	)
...
static func _set_show_full_map(setting: Dictionary) -> void:
	var setting_value: SettingValue = _parse_setting(
			setting, SHOW_FULL_MAP, VALUE_TYPE.BOOL
	)
	if setting_value.is_valid:
...
static func _set_rng_seed(setting: Dictionary) -> void:
	var setting_value: SettingValue = _parse_setting(
			setting, RNG_SEED, VALUE_TYPE.INT
	)
	var rng_seed: int
...
static func _set_palette(setting: Dictionary) -> void:
	...
	if setting_value.is_valid:
		palette = Palette.get_verified_palette(
				setting_value.dictionary_value
		)
...
static func _parse_setting(
	...
	match value_type:
		VALUE_TYPE.INT:
			_handle_int(setting[key], setting_value)
		VALUE_TYPE.BOOL:
			_handle_bool(setting[key], setting_value)
		VALUE_TYPE.STRING:
			_handle_string(setting[key], setting_value)
		VALUE_TYPE.DICTIONARY:
			_handle_dict(setting[key], setting_value)
	return setting_value


static func _handle_int(
		input_value: Variant, setting_value: SettingValue
) -> void:
	match typeof(input_value):
		TYPE_FLOAT:
			setting_value.int_value = input_value as int
			setting_value.is_valid = true


static func _handle_bool(
		input_value: Variant, setting_value: SettingValue
) -> void:
	match typeof(input_value):
		TYPE_BOOL:
			setting_value.bool_value = input_value
			setting_value.is_valid = true
		TYPE_FLOAT:
			setting_value.bool_value = input_value >= 1
			setting_value.is_valid = true


static func _handle_string(
		input_value: Variant, setting_value: SettingValue
) -> void:
	match typeof(input_value):
		TYPE_STRING:
			setting_value.string_value = input_value
			setting_value.is_valid = true


static func _handle_dict(
		input_value: Variant, setting_value: SettingValue
) -> void:
	match typeof(input_value):
		TYPE_DICTIONARY:
			setting_value.dictionary_value = input_value
			setting_value.is_valid = true
