...
func _ready() -> void:
	visible = false
	MenuScreen.set_size(self, 60, 40, 60, 60)
...
func _set_transfer_data(input_tag: StringName) -> void:
	if input_tag != InputTag.REPLAY_GAME:
		TransferData.set_rng_seed(MenuScreen.get_int(
				_ref_SeedLineEdit.text
		))
	TransferData.set_wizard_mode(MenuScreen.get_bool(
			_ref_WizardLineEdit.text
	))
	TransferData.set_show_full_map(MenuScreen.get_bool(
			_ref_MapLineEdit.text
	))
