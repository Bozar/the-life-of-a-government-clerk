...
func _connect_signals(signal_connections: Dictionary) -> void:
	...
	for source_node: String in signal_connections.keys():
		...
		for signal_name: String in signals_from_one_node.keys():
			...
			_handle_target_node(
					target_nodes, source_node, signal_name
			)
...
func _handle_target_node(
		target_nodes: Array, source_node: String, signal_name: String
) -> void:
	...
	for target_node: String in target_nodes:
		...
		target_function = get_node(target_node)[
				"_on_"
				+ Array(source_node.split("/")).pop_back()
				+ "_"
				+ signal_name
		]

		if (
				source_signal.connect(target_function)
